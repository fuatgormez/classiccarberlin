...
try{
    ...
    $db = new Database();
    $Details = $db->getRow("SELECT * FROM ".TABLE_PREFIX."TBL_NOTICES WHERE SLUG=? AND STATUS=?",[$link,"Y"]);
    $tags    = $db->getRow("SELECT * FROM ".TABLE_PREFIX."TBL_TAGS WHERE NOTICESID=?",[$Details['ID']]);
    $db->Disconnect();
                                        ...
                                        echo'<h2 class="title-heading"> '.$Details['TITLE'].' </h2>
                                            <article class="clearfix">
                                                ...
                                                <div class="post-content">
                                                    ...
                                                    <a class="read-less" href="'.URL.'/notices">'.$langDB['LANG_BACK'].'</a>
                                                </div>
