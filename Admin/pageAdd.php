...
if (isset($_POST['submit'])){
    ...
    try{
        if ($title !=NULL && $content !=NULL){
            $db = new Database();
            $querySlug = $db->getRow("SELECT SLUG FROM ".TABLE_PREFIX."TBL_PAGES WHERE SLUG=?",[$slug]);

                if ($querySlug !=NULL){
                    echo sweetalert("Bu başlıkla daha önceden sayfa oluşturulmuş lütfen bilgilerinizi kontrol ediniz!","Yönlendiriliyorsunuz...","warning","index.php?page=pageAdd","3000");
                    exit;
                }

            $insertPage = $db->insertRow("INSERT INTO ".TABLE_PREFIX."TBL_PAGES SET TITLE=?, CONTENT=?, SLUG=?, ADDDATE=?, STATUS=?",[$title,$content,$slug,$date,$status]);
            $lastID = $db->lastInsertId();

            if ($tags !=NULL){
                $insertTags = $db->insertRow("INSERT INTO ".TABLE_PREFIX."TBL_TAGS SET TAGS=?, SLUG=?, PAGESID=?, ADDDATE=?",[$tags,$slug,$lastID,$date]);
            }

            $message    = 'Sayfa ekledi! <i class="fa fa-arrow-right"></i> <a href="index.php?page=pageEdit&ID='.$lastID.'">'.$title.'</a>';
            ...
            echo sweetalert($langDB['LANG_ADDED'],$langDB['LANG_REDIRECTING'],"success","index.php?page=pagesList","2000");
            exit;
        }else{
            echo sweetalert($langDB['LANG_FAILED_ADDED'],$langDB['LANG_REDIRECTING'],"error","index.php?page=pagesList","2000");
            exit;
        }
 ...
 echo'
    ...
    <div class="row wrapper border-bottom white-bg page-heading">
        <div class="col-lg-9">
            ...
            <span class="label label-success">Sayfa Ekle</span>
        </div>
                    ...
                    <div class="ibox-content">
                        <form method="POST" class="form-horizontal" action="">
                            ...
                            <div class="form-group"><label class="col-sm-2 control-label text-success">Sayfa Başlığı <small class="text-danger">*</small></label>
                                <div class="col-sm-10"><input type="text" name="title" class="form-control" id="title" placeholder="Örn: Hakkımda" value="'.@$_SESSION['s_title'].'" required></div>
                            </div>
                            ...
                            <div class="form-group"><label class="col-sm-2 control-label text-success" data-container="body">Etiket</label>
                                <div class="col-sm-10"><input type="text" name="tags" class="form-control" value="'.@$_SESSION['s_tags'].'" placeholder="Örn: sayfa, hakkımda, bilgi Not:Etiketleri , ile ayırınız."></div>
                            </div>
                            ...
                            <div class="form-group"><label class="col-sm-2 control-label text-success" data-container="body"></label>
                                <div class="col-sm-10">
                                    ...
                                    <small> (Bu kısmı değiştirmezseniz sayfa direkt yayına girer!) </small>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                <div class="col-sm-4 col-sm-offset-2">
                                    ...
                                    <button class="btn btn-success" name="submit" type="submit">'.$langDB['LANG_BTN_SAVE'].'</button>
                                </div>
                            </div>
                        </form>
                    </div>
