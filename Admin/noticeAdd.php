...
if (isset($_POST['submit'])){
    ...
    try{
        if ($title !=NULL && $content !=NULL){
            $db = new Database();
            $querySlug = $db->getRow("SELECT SLUG FROM ".TABLE_PREFIX."TBL_NOTICES WHERE SLUG=?",[$slug]);

                if ($querySlug !=NULL){
                    echo sweetalert("Bu başlıkla daha önceden duyuru oluşturulmuş lütfen bilgilerinizi kontrol ediniz!","Yönlendiriliyorsunuz...","warning","index.php?page=pageAdd","3000");
                    exit;
                }

            $insertPage = $db->insertRow("INSERT INTO ".TABLE_PREFIX."TBL_NOTICES SET TITLE=?, CONTENT=?, SLUG=?, ADDDATE=?, STATUS=?",[$title,$content,$slug,$date,$status]);
            $lastID = $db->lastInsertId();

            if ($tags !=NULL){
                $insertTags = $db->insertRow("INSERT INTO ".TABLE_PREFIX."TBL_TAGS SET TAGS=?, SLUG=?, NOTICESID=?, ADDDATE=?",[$tags,$slug,$lastID,$date]);
            }

            $message    = 'Duyuru ekledi! <i class="fa fa-arrow-right"></i> <a href="index.php?page=noticeEdit&ID='.$lastID.'">'.$title.'</a>';
            ...
            echo sweetalert($langDB['LANG_ADDED'],$langDB['LANG_REDIRECTING'],"success","index.php?page=noticesList","2000");
            exit;
        }else{
            echo sweetalert($langDB['LANG_FAILED_ADDED'],$langDB['LANG_REDIRECTING'],"error","index.php?page=noticesList","2000");
            exit;
        }
 ...
 echo'
    ...
    <div class="row wrapper border-bottom white-bg page-heading">
        <div class="col-lg-9">
            ...
            <span class="label label-info">Duyuru Ekle</span>
        </div>
                        ...
                        <form method="POST" class="form-horizontal" action="">
                            ...
                            <div class="form-group"><label class="col-sm-2 control-label text-info" data-container="body"></label>
                                <div class="col-sm-10">
                                    ...
                                    <small> (Bu kısmı değiştirmezseniz duyuru direkt yayına girer!) </small>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                <div class="col-sm-4 col-sm-offset-2">
                                    ...
                                    <button class="btn btn-info" name="submit" type="submit">'.$langDB['LANG_BTN_SAVE'].'</button>
                                </div>
                            </div>
                        </form>
                    </div>
